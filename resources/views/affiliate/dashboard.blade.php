...
@section('title')
    <div class="flex flex-col">
        <h2 class="text-xl font-serif font-bold text-[#2C3E50]">My Dashboard</h2>
        <p class="text-xs text-gray-400">Track your affiliate performance</p>
    </div>
@endsection
...
<div class="max-w-7xl mx-auto" x-data="{ 
    ...
    <div class="flex justify-between items-center mb-8">
        ...
        <div class="bg-white px-4 py-2 rounded-xl shadow-sm border border-gray-100 flex items-center">
            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mr-2 text-right">Today:</span>
            <span class="text-xs font-bold text-[#7D2E35]">{{ date('M j, Y') }}</span>
        </div>
    </div>
    ...
    <div class="relative bg-[#7D2E35] rounded-[2rem] p-10 text-white mb-10 overflow-hidden shadow-2xl group">
        ...
        <div class="relative z-10">
            <div class="flex items-center mb-4">
                ...
                <span class="text-[10px] font-bold uppercase tracking-[0.2em] text-white/70">Welcome back</span>
            </div>
            <h1 class="font-serif text-3xl md:text-4xl font-bold mb-4 leading-tight">
                Welcome, may your efforts be <span class="italic text-[#E8D5B5]">meaningful and blessed.</span>
            </h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <div class="bg-surface rounded-2xl p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:-translate-y-1 transition-transform duration-300 border border-gray-50 flex items-center">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-[#2C3E50]">Rp {{ number_format($totalEarnings/1000000, 1) }}M</h3>
            </div>
        </div>
        ...
        <div class="bg-surface rounded-2xl p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:-translate-y-1 transition-transform duration-300 border border-gray-50 flex items-center">
            ...
            <div>
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Total Clicks</p>
        ...
        <div class="bg-surface rounded-2xl p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:-translate-y-1 transition-transform duration-300 border border-gray-50 flex items-center">
            ...
            <div>
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Successful Orders</p>
        ...
        <div class="bg-surface rounded-2xl p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:-translate-y-1 transition-transform duration-300 border border-gray-50 flex items-center">
            ...
            <div>
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Available Balance</p>
                <h3 class="text-xl font-bold text-[#2C3E50]">Rp {{ number_format($availableBalance/1000000, 1) }}M</h3>
            </div>
        </div>
    </div>
    ...
    <div class="bg-surface rounded-3xl p-8 shadow-sm border border-gray-50 mb-10">
        <div class="flex items-start mb-8">
            ...
            <div>
                <h3 class="text-lg font-bold text-[#2C3E50] mb-1">Your Affiliate Link</h3>
                <p class="text-xs text-gray-400 font-medium tracking-wide">Share this link to earn commissions</p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <button class="bg-[#7D2E35] hover:bg-[#632429] text-white px-8 py-4 rounded-2xl font-bold text-sm transition-all shadow-lg flex items-center"
                    @click="navigator.clipboard.writeText(promoLink); alert('Link copied!');">
    ...
    <div class="bg-surface rounded-3xl shadow-sm border border-gray-50 overflow-hidden">
        <div class="p-8 border-b border-gray-50 flex justify-between items-center">
            <div>
                <h3 class="text-lg font-bold text-[#2C3E50] mb-1">Commission History</h3>
                <p class="text-xs text-gray-400 font-medium tracking-wide">Track your earnings transparently</p>
            </div>
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" class="bg-white border border-gray-100 px-4 py-2 rounded-xl text-[10px] font-bold text-gray-500 flex items-center hover:border-gray-200 transition-colors">
                    All Status <i class="fas fa-chevron-down ml-2 text-[8px]"></i>
                </button>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        <td class="px-8 py-6">
                            <p class="text-xs font-bold text-[#2C3E50] mb-1">{{ $commission->created_at->format('M j, Y') }}</p>
                            <p class="text-[9px] text-gray-400 font-medium">{{ $commission->created_at->format('h:i A') }}</p>
                        </td>
                        <td class="px-8 py-6">
                            <p class="text-xs font-bold text-[#2C3E50]">{{ $commission->order->product->name ?? 'Unknown' }}</p>
                        </td>
                        <td class="px-8 py-6">
                    ...
                    <tr>
                        <td colspan="5" class="py-20 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-xs text-gray-400 font-bold">No commission history found</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="p-8 border-t border-gray-50 flex items-center justify-between">
            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Showing 1-{{ $recentCommissions->count() }} of {{ $recentCommissions->count() }} results</p>
            <div class="flex gap-2">
                <!-- Mock Pagination to match SS -->
