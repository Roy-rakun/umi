...
<html lang="id" class="h-full">
 ...
 <body class="h-full font-body" style="background-color: var(--color-bg); color: var(--color-text);">
  <div id="app-wrapper" class="w-full h-full overflow-auto">
   ...
   <section class="min-h-screen flex items-center section-padding pt-24 md:pt-32">
    <div class="max-w-7xl mx-auto w-full">
     <div class="grid md:grid-cols-2 gap-12 md:gap-16 items-center"><!-- Left: Image -->
      <div class="animate-fade-in-up order-2 md:order-1">
       <div class="relative">
        <div class="aspect-[3/4] rounded-3xl overflow-hidden card-soft max-w-md mx-auto">
         <div class="w-full h-full bg-gradient-to-br from-rose-100 to-amber-50 flex items-center justify-content-center p-8">
          @if(isset($sections['hero']->content['image_url']))
           <img src="{{ $sections['hero']->content['image_url'] }}" alt="Hero Image" class="w-full h-full object-cover">
          @else
      ...
      <div class="order-1 md:order-2 text-center md:text-left">
       ...
       <h1 class="font-heading text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-6 animate-fade-in-up delay-100" style="color: var(--color-primary);">{{ $sections['hero']->content['headline'] }}</h1>
       <div class="text-lg md:text-xl leading-relaxed mb-10 opacity-80 animate-fade-in-up delay-200">
           {!! $sections['hero']->content['tagline'] !!}
       </div>
       <div class="flex flex-col sm:flex-row gap-4 justify-center md:justify-start animate-fade-in-up delay-300">
           <a href="{{ $sections['hero']->content['primary_url'] ?? '#products' }}" class="btn-primary px-8 py-4 rounded-2xl font-semibold text-center"> {{ $sections['hero']->content['primary_button'] }} </a> 
           <a href="{{ $sections['hero']->content['secondary_url'] ?? '#about' }}" class="btn-secondary px-8 py-4 rounded-2xl font-semibold text-center"> {{ $sections['hero']->content['secondary_button'] }} </a>
       </div>
      </div>
     </div>
    </div>
   ...
   <section class="section-padding" style="background: linear-gradient(180deg, #fef5f3 0%, #fff7f6 100%);">
    <div class="max-w-6xl mx-auto">
     <h2 class="font-heading text-3xl md:text-4xl font-bold text-center mb-16" style="color: var(--color-primary);">{{ $sections['problem']->content['title'] }}</h2>
     <div class="grid md:grid-cols-3 gap-8">
      @foreach(['card_1', 'card_2', 'card_3'] as $idx => $cardKey)
      @php $item = $sections['problem']->content[$cardKey] ?? null; @endphp
      @if($item)
   ...
   <section id="about" class="section-padding">
    <div class="max-w-7xl mx-auto">
     <div class="grid md:grid-cols-2 gap-12 md:gap-16 items-center"><!-- Left: Content -->
      <div><span class="inline-block px-4 py-2 rounded-full text-sm font-medium mb-6" style="background-color: #fef5f3; color: var(--color-primary);">{{ $sections['about']->content['badge'] }}</span>
       <h2 class="font-heading text-3xl md:text-4xl font-bold mb-6" style="color: var(--color-primary);">{{ $sections['about']->content['title'] }}</h2>
       <div class="space-y-4 leading-relaxed opacity-80">
        {!! $sections['about']->content['description'] !!}
       </div>
       <div class="flex items-center gap-6 mt-8">
          @foreach($sections['about']->content['stats'] as $stat)
   ...
   <section class="section-padding" style="background: linear-gradient(180deg, #fef5f3 0%, #fff7f6 100%);">
    <div class="max-w-4xl mx-auto text-center">
     ...
     <h2 class="font-heading text-3xl md:text-4xl font-bold mb-8" style="color: var(--color-primary);">{{ $sections['explanation']->content['title'] }}</h2>
     <div class="space-y-6 leading-relaxed opacity-80 mb-10">
      {!! $sections['explanation']->content['description'] !!}
     </div><a href="{{ $sections['explanation']->content['button_url'] ?? '#products' }}" class="btn-primary inline-block px-10 py-4 rounded-2xl font-semibold"> {{ $sections['explanation']->content['button_text'] }} </a>
    </div>
   </section>
   @endif
    ...
    <div class="max-w-7xl mx-auto">
     ...
     <div x-data="{ open: false, product: {} }">
      ...
      <div class="text-center mt-12">
          <a href="{{ route('products.index') }}" class="btn-secondary px-8 py-3 rounded-xl font-bold inline-block">Lihat Semua Produk</a>
      </div>
            ...
            <div class="grid md:grid-cols-2">
                ...
                <div class="p-8 flex flex-col">
                    ...
                    <div class="mt-auto border-t pt-6 flex items-center justify-between">
                        ...
                        <a :href="'/checkout/' + product.product_id" class="btn-primary px-8 py-3 rounded-xl font-bold">
   ...
   <!-- Affiliate Section -->
   @if(isset($sections['affiliate']))
   <section id="affiliate" class="section-padding" style="background: linear-gradient(180deg, #f5ebe8 0%, #f8f0ed 100%);">
    <div class="max-w-5xl mx-auto">
     <div class="text-center mb-12"><span class="inline-block px-4 py-2 rounded-full text-sm font-medium mb-4" style="background-color: #fff7f6; color: var(--color-primary);">{{ $sections['affiliate']->content['badge'] }}</span>
      <h2 class="font-heading text-3xl md:text-4xl font-bold mb-4" style="color: var(--color-primary);">{{ $sections['affiliate']->content['title'] }}</h2>
      <p class="max-w-2xl mx-auto opacity-80">{{ $sections['affiliate']->content['description'] }}</p>
     </div>
     <div class="grid md:grid-cols-3 gap-6 mb-10">
      @foreach(['card_1', 'card_2', 'card_3'] as $idx => $cardKey)
      @php $feature = $sections['affiliate']->content[$cardKey] ?? null; @endphp
      @if($feature)
     ...
     <div class="flex flex-col sm:flex-row gap-4 justify-center">
      <a href="{{ $sections['affiliate']->content['register_url'] ?? route('register') }}" class="btn-primary px-8 py-4 rounded-2xl font-semibold text-center"> {{ $sections['affiliate']->content['register_button'] }} </a> 
      <a href="{{ $sections['affiliate']->content['login_url'] ?? route('login') }}" class="btn-secondary px-8 py-4 rounded-2xl font-semibold text-center"> {{ $sections['affiliate']->content['login_button'] }} </a>
     </div>
    </div>
   </section>
   @endif
   ...
   <section class="section-padding">
    <div class="max-w-6xl mx-auto">
     <div class="text-center mb-16"><span class="inline-block px-4 py-2 rounded-full text-sm font-medium mb-4" style="background-color: #fef5f3; color: var(--color-primary);">{{ $sections['testimonials']->content['badge'] }}</span>
      <h2 class="font-heading text-3xl md:text-4xl font-bold" style="color: var(--color-primary);">{{ $sections['testimonials']->content['title'] }}</h2>
     </div>
     <div class="grid md:grid-cols-3 gap-8">
      @foreach($sections['testimonials']->content['items'] as $testimonial)
   ...
   <section id="contact" class="section-padding" style="background: linear-gradient(180deg, #fef5f3 0%, #fff7f6 100%);">
    <div class="max-w-4xl mx-auto text-center">
     <h2 class="font-heading text-3xl md:text-4xl font-bold mb-4" style="color: var(--color-primary);">{{ $sections['contact']->content['title'] }}</h2>
     <p class="opacity-80 mb-10">{{ $sections['contact']->content['description'] }}</p>
     <div class="flex flex-wrap justify-center gap-4 mb-8">
        @foreach($sections['contact']->content['channels'] ?? [] as $channel)
     ...
     <p class="text-sm opacity-60 card-soft inline-block px-4 py-2 rounded-xl">⚠️ {{ $sections['contact']->content['warning_text'] }}</p>
    </div>
   </section>
   @endif
   ...
   <footer class="py-8 px-6" style="background-color: #f0e6e3;">
    <div class="max-w-6xl mx-auto">
     <div class="flex flex-col md:flex-row items-center justify-between gap-6">
      ...
      <div class="flex flex-wrap justify-center gap-6 text-sm opacity-70">
          @foreach($sections['footer']->content['links'] as $link)
          <a href="{{ $link['url'] }}" class="hover:opacity-100 transition-opacity">{{ $link['name'] }}</a>
          @endforeach
      </div>
      <p class="text-sm opacity-60">{{ $sections['footer']->content['copyright'] }}</p>
     </div>
    </div>
   </footer>
   @endif
